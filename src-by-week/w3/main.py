...
import argparse
from src.global_utils import make_dir, plot_sales_data, get_file_name
import json
...
class DP(DataProcessor):
    ...
    def aggregate(self, column_name: str) -> float:
        ...
        self._db.insert(
            process_id,
            start_time=datetime.datetime.now(datetime.UTC).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            file_name=self._file_name,
            file_path=self._fp,
            description=inspect.stack()[0][3],
        )
        ...
        for row_num, row in enumerate(tqdm(data_reader_gen)):
            if (
                isinstance(self._n_rows, int)
                and isinstance(row_num, int)
                and row_num % 10000 == 0
            ):
                self._db.update_percentage(
                    process_id=process_id, percentage=100 * row_num / self._n_rows
                )
        ...
        self._db.update_end_time(
            process_id=process_id,
            end_time=datetime.datetime.now(datetime.UTC).strftime("%Y-%m-%d %H:%M:%S"),
        )
        return aggregate
...
def revenue_per_region(dp: DP) -> Dict:
    ...
    dp.get_db().insert(
        process_id=process_id,
        start_time=datetime.datetime.now(datetime.UTC).strftime("%Y-%m-%d %H:%M:%S"),
        file_name=dp.get_file_name(),
        file_path=dp.get_file_path(),
        description=inspect.stack()[0][3],
    )

    for n_row, row in enumerate(tqdm(data_reader_gen)):
        if (
            isinstance(dp.get_n_rows(), int)
            and isinstance(n_row, int)
            and n_row % 10000 == 0
        ):
            dp.get_db().update_percentage(
                process_id=process_id, percentage=100 * n_row / dp.get_n_rows()
            )
