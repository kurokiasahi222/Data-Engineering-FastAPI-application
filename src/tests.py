...
class TestApp(unittest.TestCase):
    ...
    def test_health(self):
        ...
        self.assertEqual(
            response.json(),
            {"status": "ok", "MODE": MODE}, 
            msg="Health API check failed"
        )

    def test_processes(self):
        response = self.client.get("/processes")
        self.assertEqual(response.status_code, 200, msg="Response code should be 200")
        self.assertIsInstance(response.json(), list,msg="Instance of processes object should be list")
        if len(response.json()) > 0:
            self.assertTrue(
                all([isinstance(process, dict) for process in response.json()]),
                msg="Process data type should de dictionary",
            )

            response_model_ex = ProcessStatus(
                process_id="",
                file_name="",
                file_path="",
                description="",
            ...
            self.assertTrue(
                all(
                    [
                        process.keys() == response_model_ex.dict().keys()
