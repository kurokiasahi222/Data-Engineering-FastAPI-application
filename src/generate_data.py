...
class DataGenerator:
    ...
    def read_seed_data(self):
        with open(
            os.path.join(
                ...
                constants.SEED_FOLDER_NAME,
                constants.SEED_FILE_NAME,
            )
        ) as f:
        ...
        for stock_no, seed_data in items.items():
            ...
            if all(
                [
                    (
                        isinstance(
                            seed_data[constants.SeedDataColNames.DESCRIPTION], str
                        )
                        and len(seed_data[constants.SeedDataColNames.DESCRIPTION]) > 0
                    ),
                    (
                        self.can_typecast_to_float(
                            seed_data[constants.SeedDataColNames.UNIT_PRICE]
                        )
                        and float(seed_data[constants.SeedDataColNames.UNIT_PRICE]) > 0
                    ),
                ]
            ):
    ...
    def generate_data(self, start_date, end_date):
        ...
        data = {
            constants.SeedDataColNames.DESCRIPTION: row[
                constants.SeedDataColNames.DESCRIPTION
            ],
            constants.SeedDataColNames.UNIT_PRICE: row[
                constants.SeedDataColNames.UNIT_PRICE
            ],
            constants.SeedDataColNames.UNITS: n_units[0],
            constants.SeedDataColNames.TOTAL: row[constants.SeedDataColNames.UNIT_PRICE]
            * n_units[0],
            constants.SeedDataColNames.STOCK_NO: row[
                constants.SeedDataColNames.STOCK_NO
            ],
            constants.SeedDataColNames.COUNTRY: country[0],
            constants.SeedDataColNames.INVOICE_NO: str(uuid.uuid1()),
            constants.SeedDataColNames.DATE: self.random_date(
                start_date, end_date
...
def generate_file(data_gen_obj, file_details):
    ...
    make_dir(
        directory=os.path.join(
            CURRENT_FOLDER, constants.DATA_FOLDER_NAME, file_details["folder_name"]
        )
    )
    ...
    with open(file_path, "a") as f:
        for n_row in tqdm(range(file_details["n_datapoints"])):
            row = data_gen_obj.generate_data(
                start_date=file_details["start_date"], end_date=file_details["end_date"]
            )
            f.write(
                ",".join(
                    [
                        str(row[constants.SeedDataColNames.STOCK_NO]),
            ...
            if n_row % 10000 == 0:
                tqdm.write(
                    f"Total units : {human_readable(int(total_units))}, "
    ...
    tqdm.write(
        f"File Name : {file_details['file_name']}, Total units : {int(total_units)}, "
